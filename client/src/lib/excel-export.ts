...
export async function exportToExcel(
  ...
  // Add header with ZatScan branding
  worksheet.getCell('A1').value = 'ZatScan - ZATCA QR Code Scanner';
  worksheet.getCell('A1').font = { bold: true, size: 16, color: { argb: 'FFFFFFFF' } };
  worksheet.getCell('A1').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4472C4' } };
  worksheet.mergeCells('A1:J1');
  
  worksheet.getCell('A2').value = 'Saudi Arabia Invoice QR Code Analysis Report';
  worksheet.getCell('A2').font = { bold: true, size: 12, color: { argb: 'FFFFFFFF' } };
  worksheet.getCell('A2').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF70AD47' } };
  worksheet.mergeCells('A2:J2');
  
  // Add company info
  worksheet.getCell('A4').value = 'Generated by ZatScan™';
  worksheet.getCell('A4').font = { bold: true };
  worksheet.getCell('H4').value = 'Export Date:';
  worksheet.getCell('I4').value = new Date().toLocaleDateString();
  
  worksheet.getCell('A5').value = 'Website: www.growthsphereindustries.com';
  worksheet.getCell('H5').value = 'Export Time:';
  worksheet.getCell('I5').value = new Date().toLocaleTimeString();
  
  worksheet.getCell('A6').value = 'Professional ZATCA QR Code Analysis System';
  worksheet.getCell('H6').value = 'Version: 1.0';
  
  // Add report summary
  worksheet.getCell('A8').value = 'Report Summary:';
  worksheet.getCell('A8').font = { bold: true };
  
  const validQRs = dataToExport.filter(qr => qr.status === 'valid');
  worksheet.getCell('A9').value = 'Total Records:';
  worksheet.getCell('B9').value = dataToExport.length;
  worksheet.getCell('D9').value = 'Valid Records:';
  worksheet.getCell('E9').value = validQRs.length;
  worksheet.getCell('G9').value = 'Invalid Records:';
  worksheet.getCell('H9').value = dataToExport.length - validQRs.length;
